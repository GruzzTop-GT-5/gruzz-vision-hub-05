...
export default function CreateOrder() {
  ...
  const onSubmit = async (data: OrderFormData) => {
    ...
    try {
      ...
      // Create description with compressor details if applicable
      let fullDescription = `${data.description}\n\n📍 Адрес: ${data.address}\n💰 Оплата: ${data.hourly_rate}₽/час\n⏱ Продолжительность: ${data.work_hours} ч`;
      
      if (data.compressor_rent && compressorData) {
        fullDescription += `\n\n🔧 Аренда компрессора:\n• Время: ${compressorData.totalHours} ч\n• Локация: ${
          compressorData.location === 'city' ? 'В городе' : 
          compressorData.location === 'suburb' ? 'Загородом' : 
          'Далеко (договорное время)'
        }\n• Оплата: ${compressorData.paymentType === 'cash' ? 'Наличные' : 'С НДС'}\n• Стоимость: ${compressorData.totalPrice.toLocaleString('ru-RU')} ₽`;
      }
      ...
      const { data: orderData, error: orderError } = await supabase
        .from('orders')
        .insert({
          // Генерируем номер с правильным типом сервиса
                        ...
                        <div className="space-y-1 leading-none flex-1">
                          ...
                          {compressorData && (
                            <div className="text-xs text-primary mt-2">
                              ✓ {compressorData.totalHours} ч • {compressorData.totalPrice.toLocaleString('ru-RU')} ₽
                            </div>
                          )}
                        </div>
                      </FormItem>
