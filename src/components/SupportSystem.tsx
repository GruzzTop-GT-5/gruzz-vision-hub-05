...
export const SupportSystem = () => {
  ...
  const createSupportTicket = async () => {
    ...
    try {
      ...
      // Generate temporary ticket number that will be replaced by trigger
      ...
      const { data: ticketData, error: ticketError } = await supabase
        ...
        .insert([{
          ...
          created_by: user.id,
          status: 'open',
          ticket_number: tempTicketNumber // Will be replaced by trigger
        }])
        .select()
        .single();
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          filteredTickets.map((ticket) => (
            <Card key={ticket.id} className="card-steel p-6 hover:bg-steel-700 hover:scale-105 transition-all duration-300 cursor-pointer">
              <div 
                className="space-y-4"
