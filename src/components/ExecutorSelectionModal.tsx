...
interface ExecutorSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelectExecutor: (userId: string) => void;
  title?: string;
}

export const ExecutorSelectionModal = ({ 
  isOpen, 
  onClose, 
  onSelectExecutor,
  title = "Выбрать исполнителя" 
}: ExecutorSelectionModalProps) => {
  ...
  const handleSelectExecutor = (userId: string) => {
    onSelectExecutor(userId);
    onClose();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="card-steel-dialog max-w-md">
        <DialogHeader>
          <DialogTitle className="text-steel-100">{title}</DialogTitle>
        </DialogHeader>
              ...
              users.map((profile) => (
                <Card key={profile.id} className="card-steel p-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <Button
                      size="sm"
                      onClick={() => handleSelectExecutor(profile.id)}
                      className="shrink-0"
                    >
